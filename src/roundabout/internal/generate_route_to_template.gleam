...
pub fn generate_route_to_template_rec(
  ancestors: List(Info),
  node: Node,
) -> Document {
    ...
    False -> {
      ...
      let sub_types =
        list.map(node.sub, fn(node) {
          generate_route_to_template_rec(next_ancestors, node)
        })

      let this = generate_route_to_template(ancestors, node)
...
pub fn generate_route_to_template(ancestors: List(Info), node: Node) -> Document {
  let is_root = list.is_empty(ancestors)
  let next_ancestors = list.prepend(ancestors, node.info)

  let route_to_template_cases =
    node.sub
    |> list.map(generate_route_to_template_case(is_root, next_ancestors, _))
    |> doc.join(doc.line)

  let function_name =
    [common.get_function_name(ancestors, node.info), "route_to_template"]
...
fn generate_route_to_template_case(
  is_root: Bool,
  ancestors: List(Info),
  node: Node,
) -> Document {
  let variant_params_str =
    node.info.path
...
pub fn get_branch_result(
  ...
  // Add `<> some_route_to_template(sub)`
  |> fn(self) {
    case list.is_empty(node.sub) {
      True -> self
      False ->
